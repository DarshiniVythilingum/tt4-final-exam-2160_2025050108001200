...
2. What does `docker images` command show?
   ...
   - **C) Downloaded images**
   - D) Running apps
     //When I type docker images, I see the list of downloaded images since beginning of this semester.
...
3. How do you remove a Docker container?
   ...
   - C) `docker remove`
   - **D) `docker rm`**
     //Use that during midterm to clear all previous containers.
...
4. What port does Docker map to when using `EXPOSE 80`?
   ...
   - C) Always port 8080
   - D) The backend’s IP
     //You said that 80 is the container's internal port during class.
...
5. Which command initializes a new Git repository?
   ...
   - **C) `git init`**
   - D) `git open`
     //1st command before creating the README.md in c drive when I create a folder.
...
6. What is a Git branch?
   ...
   - D) A type of commit
     //Used it for team projects to avoid messing original code / testing.
...
7. What does `git pull` do?
   ...
   - D) Backup your files
     //I use pull to retrieve latest changes and merge to my existing code.
...
8. How do you stage all modified files?
   ...
   - C) `git commit -m`
   - **D) `git add .`**
     //add . does the job of including all modifies files.
...
9. What is Entity Framework Core used for?
   ...
   - D) Running JavaScript
     //I had to use [using Microsoft.EntityFrameworkCore;] in nearly all my classes related to database.
...
- D) Handle routing
  //I remember adding connection strings in appsettings.json.
...
- **C) `dotnet run`**
- D) `dotnet execute`
  //That is the command I use to run the project.
...
- D) A redirect
  //I remember that HttpPost has something to do with POST requests.
...
- **C) `ng new`**
- D) `angular init`
  //You showed that in class.

14. In React, what is a component?

- A) A database table -**B) A function or class that renders UI**
- C) A hook
- D) A node module
  //Same explanation as in simulation as for Angular which deals with UI.
...
- **C) `v-for:value`**
- D) `@value`
  //Honestly, I have no idea, I think it is C), I don't remember a lot from VueJS in class.
...
- D) Loading database drivers
  //easy way to interact btw client and server.

19. What is the role of `Tailwind CSS`?

- A) It’s a database system
- B) A backend tool for API management -**C) A utility-first CSS framework**
- D) A JavaScript compiler
  //Tailwind CSS seems front-end involving UI so C) would seems the closer answer compared to the other backend answers.
...
- D) Clears Docker images
  //Used that command to shut down all containers before shutting my device.
